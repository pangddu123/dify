...
| Phase 1 | 响应级聚合节点 `response-aggregator` | 5–7 天 | 节点能在画布上拖出、运行、出聚合结果（workflow + chat 两套验收） |
...
### 5.1 节点语义
- **画布拓扑**：`Start → [LLM-A, LLM-B, LLM-C] → ResponseAggregator → End/Answer`
...
```
api/core/workflow/nodes/response_aggregator/
├── __init__.py
├── node.py                 # ResponseAggregatorNode(Node)
├── entities.py             # ResponseAggregatorNodeData (Pydantic)
...
class ResponseAggregatorNodeData(BaseNodeData):
    type: NodeType = RESPONSE_AGGREGATOR_NODE_TYPE   # 包级常量 "response-aggregator"；✅ Phase 0 Q1 已验证
    ...
    def _check_source_id_unique(self) -> "ResponseAggregatorNodeData":
        seen: set[str] = set()
...
class ResponseAggregatorNode(Node[ResponseAggregatorNodeData]):
    node_type = "response-aggregator"   # ⚠️ Phase 0 Q1 验证
...
```
web/app/components/workflow/nodes/response-aggregator/
...
| # | 文件 | 用途 | 必填 |
|---|---|---|---|
| 1 | `web/app/components/workflow/types.ts` | `BlockEnum` 加 `ResponseAggregator = 'response-aggregator'` | ✅ |
...
### 5.6 后端注册改动
- 节点放进 `api/core/workflow/nodes/response_aggregator/` 即被 `_import_node_package` 自动发现。
...
│   ├── __init__.py
│   ├── response_level.py       # ResponseLevelRunner — 包 P1 response-aggregator 现有逻辑（P2.6.5）
...
`pkgutil.walk_packages` 在 `_import_node_package` 里递归扫描，所有 `@register_*` 装饰器自动生效；P1 `response-aggregator` 节点保留为"响应级 fast path"**不删**——它是 backwards-compat 的着陆点。
...
`response-aggregator` 现有响应级语义；并发调 `backend.generate`，收齐喂
...
| 层级 | 文件 | 内容 |
|---|---|---|
| 单测 | `api/tests/unit_tests/core/workflow/nodes/response_aggregator/` | 策略 + 节点 |
...
| 前端单测 | `web/app/components/workflow/nodes/response-aggregator/__tests__/` | panel + use-config |
...
- **测试兜底前置**：P1.4 仅保留策略 / 节点层完整测试；`tests/unit_tests/core/workflow/nodes/response_aggregator/test_entities.py` 本轮补 14 条 schema mini-tests（valid 2/path-segments/short/empty/blank-seg/empty-seg/blank-source/empty-source/extra-field/defaults/too-few/duplicate/concat-config/unknown-strategy），首跑 14/14 绿
...
- §5.3 `ResponseAggregatorNodeData`：
...
- `response_aggregator/__init__.py` 引入包级常量 `RESPONSE_AGGREGATOR_NODE_TYPE = "response-aggregator"`（仿 `knowledge_index/__init__.py:3`），后续 `entities.py` / `node.py` / 注入分支都引常量而非字面量，减手误
- **P1.1 landing 已完成**：`api/core/workflow/nodes/response_aggregator/{__init__.py, entities.py, exceptions.py}` 三文件落地；4 条验收（正常实例化 / min_length 拒 / 异常可导入 / 重复 source_id 拒）全绿
