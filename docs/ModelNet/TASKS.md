...
### P3.A.3 ensemble_aggregator 测试翻译 + 新增（1d）

依赖：P3.A.1 + P3.A.2

- 后端：dynamic weight 三分支（成功 / 失败-无 fallback 抛 `WeightResolutionError` / 失败-有 fallback 回退 + trace warning）
- 后端：`weighted_majority_vote` 单测（权重并列 / 单边强权 / 全 1 退化为 v2.4 majority_vote）
- 后端：`SourceAggregationContext` 不含 backend 语义（确认 ResponseAggregator 不依赖 backends/capabilities，回归 Rv3-9）
- 前端：input-list weight + fallback_weight 输入框 + 变量引用 picker；strategy-selector 三策略切换 + ui_schema 反射
