...
### P2.2 LlamaCppBackend (强制 ssrf_proxy) + sample yaml + dify_config 配置项
...
- `parallel_ensemble/backends/llama_cpp.py`：`LlamaCppBackend(ModelBackend)`，`spec_class = LlamaCppSpec`，`@register_backend("llama_cpp")`
  ...
  - `validate_requirements(spec, requirements) -> list[ValidationIssue]`：默认实现按 capability 兜底；`min_top_k` 任意值放行（llama.cpp 无硬上限）
  - `generate(prompt, params)`：`/completion?max_tokens=...&stream=false` 走 `core.helper.ssrf_proxy`（ADR-8）
  - `generate_stream(prompt, params)`：SSE
  - `step_token(prompt, top_k)`：`/completion?max_tokens=1&n_probs=k&post_sampling_probs=true`，响应解析独立函数（R7：固化 `top_probs` schema）+ 单测
  - `apply_template(messages)`：`/apply-template`
- 在 `api/configs/feature/__init__.py`（或 sub config）加 `MODEL_NET_REGISTRY_PATH: str = "api/configs/model_net.yaml"` Field（让运维可 env 覆盖）
- 写 sample `api/configs/model_net.yaml.example` 模板（含 `backend: llama_cpp` 字段示例）+ 同目录 README 说明字段（**真实 yaml 进 `.gitignore`**，避免提交内网 URL）

### P2.2.4 BACKEND_CAPABILITIES.md（v2.4 新任务）

把 EXTENSIBILITY_SPEC §3.2 capability 矩阵 + 三个语义坑（POST_SAMPLING_PROBS vs LOGITS_RAW / OpenAI top_logprobs ≤ 20 / vLLM logprobs 是 log-softmax 需 exp 归一）钉成独立文档 `docs/ModelNet/BACKEND_CAPABILITIES.md`，附 fixture（每个 backend 的 capability 声明对照 spec），是 Phase 4 加 backend 时的合约文件。

### P2.3 模型注册表 + LlamaCppBackend 单测（v2.4 扩展）

`api/tests/unit_tests/core/workflow/nodes/parallel_ensemble/llama_cpp/`：

- `test_registry_load`: 临时写 yaml 文件（含 `backend: llama_cpp`），验证 `ModelRegistry.instance()` 通过 BackendRegistry 反查 `LlamaCppSpec` 加载正确
- `test_extra_forbid`: yaml entry 含未知字段 → `RegistryFileError`（包装 Pydantic `ValidationError`）
- `test_unknown_backend`: yaml `backend: my_zmq` 但未注册 → `RegistryFileError("backend 'my_zmq' is not registered; known: [...]")`
- `test_unknown_alias`: `get("nope")` raises `UnknownModelAliasError`
- `test_list_aliases_returns_backend_info`: 返回 `BackendInfo` dict，含 `id / backend / model_name / capabilities / metadata`，**不含 url / api_key**（关键 SSRF 防护，T2）
- `test_capability_declaration`: `LlamaCppBackend.capabilities(spec)` 含 `STREAMING + TOKEN_STEP + TOP_PROBS + POST_SAMPLING_PROBS + CHAT_TEMPLATE`，不含 `LOGITS_RAW`
- `test_validate_requirements_default`: 默认 capability-bottom 兜底通过 / 不通过路径
- `test_step_token_uses_ssrf_proxy`: monkeypatch `ssrf_proxy.post`，验证被调用 + body 含 `max_tokens=1, n_probs=k, post_sampling_probs=true` + timeout 正确换算
- `test_generate_uses_ssrf_proxy`: 同上，验证 `/completion` 端点 + stream=false 路径
- `test_apply_template_uses_ssrf_proxy`: `/apply-template` 端点
