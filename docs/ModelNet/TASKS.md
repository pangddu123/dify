...
> **v3 升级**（2026-04-29）：基于 v2.4 P1.1–P2.12 已落地代码（完全保留，不重写）做：(1) 升级 `ensemble_aggregator`（response 加权 + 动态 selector + `ResponseAggregator` / `SourceAggregationContext` SPI 切分）；(2) 重定位 `parallel_ensemble`（aggregator-as-executor，删 `model_aliases`+`question_variable`，加 `token_sources`，扩 `ModelBackend.step_token` 至 `TokenStepParams`）；(3) 新增 `token-model-source` 节点。新增 11 个子任务（P3.0 / P3.A.1-3 / P3.B.0-5 / P3.C.1-2 + 2 个 ship 检查点，~15d 工时）；v2.4 DSL 不向上兼容（ADR-v3-13）；v2.4 Phase 3/4 在 v3 ship B 后衔接。
...
**回归**：`uv run --project . pytest tests/unit_tests/core/workflow/nodes/parallel_ensemble/ tests/unit_tests/core/workflow/nodes/ensemble_aggregator/ -q -o addopts=""` → **147/147 绿**（既有 71 + P1 47 + **P2.5 新增 29**）；ruff / basedpyright 全绿。
...
### ✅ P2.6.5 ResponseLevelRunner（v2.4 新任务，包 P1 ensemble-aggregator 现有逻辑）(2026-04-28)
...
**回归**：`uv run --project api pytest api/tests/unit_tests/core/workflow/nodes/parallel_ensemble/ api/tests/unit_tests/core/workflow/nodes/ensemble_aggregator/ -q -o addopts=""` → **177/177 绿**（既有 116 + P1 47 + **P2.6.5 新增 14**：12 主体 + 2 review-fix 回归覆盖 elapsed_ms 时序 / params 共享）；ruff 全绿；basedpyright 0 errors。
...
**回归**：`uv run --project api pytest api/tests/unit_tests/core/workflow/nodes/parallel_ensemble/ api/tests/unit_tests/core/workflow/nodes/ensemble_aggregator/ -q -o addopts=""` → **190/190 绿**（既有 177 + **P2.7 新增 13**）；ruff check / format --check 全绿；项目级 basedpyright 0 errors（pyrightconfig.json 默认 exclude `tests/`）。
...
**回归**：`uv run pytest tests/unit_tests/core/workflow/nodes/parallel_ensemble/ tests/unit_tests/core/workflow/nodes/ensemble_aggregator/ tests/unit_tests/core/workflow/test_node_factory.py -q -o addopts=""` → **227/227 绿**（parallel_ensemble 143 + ensemble_aggregator Phase 1 47 + test_node_factory 35 + 新增 2）；`uv run ruff check / format --check core/workflow/node_factory.py tests/unit_tests/core/workflow/test_node_factory.py` 全绿；`uv run basedpyright <changed files>` → 0 errors / 0 warnings / 0 notes
...
**关键测试 pattern**：
- `_make_node` 走 `ParallelEnsembleNode.__new__` 绕过 `Node.__init__`，只注入 `_run` 实际读的 7 个属性，与 `ensemble_aggregator/test_node.py` 对称（factory 链路 6 kwargs 注入身份验证已由 P2.9 的 `test_creates_parallel_ensemble_node` 钉死）
...
> **三件事**：
> 1. **升级** `ensemble_aggregator`（response 模式）→ 加 `weight`（含动态 `VariableSelector`） / `fallback_weight` / `extra` + `ResponseAggregator` SPI 切至 `SourceAggregationContext` + `weighted_majority_vote` 策略
...
- ✅ `EXTENSIBILITY_SPEC.md` §3 加注："response_level runner v3 删除，response 模式统一走 ensemble-aggregator + ResponseAggregator SPI"
- ✅ 不写 `MIGRATION_v2_to_v3.md`（ADR-v3-13）

### ✅ P3.A.1 ensemble_aggregator 后端升级（response 模式）（1.5d，2026-04-30）
...
### ✅ P3.A.2 ensemble_aggregator 前端升级（1.5d，2026-04-30）

依赖：P3.A.1

- ✅ `types.ts`：`AggregationInputRef` 加 `weight`（`number | ValueSelector`）/ `fallback_weight: number | null` / `extra: Record<string, unknown>`；**不**加 `top_k_override`（ADR-v3-6）；strategy 字面量从 2 个扩到 3 个；新增 `ENSEMBLE_STRATEGY_META` 静态镜像后端 `list_strategies()` 输出（majority_vote `{}` / concat `{separator, include_source_label, order_by_weight}` / weighted_majority_vote `{}`），`UiSchema` 走 `parallel-ensemble/types` 单一来源
...
- ✅ `use-config.ts` 新增 `handleWeightChange(number | ValueSelector)` / `handleFallbackWeightChange(number | null)` / `filterNumericVar`；`handleAddInput` 默认 `weight=1, fallback_weight=null, extra={}`；`handleStrategyConfigChange` 改接全量 `EnsembleStrategyConfig`（`DynamicConfigForm.handlePatchKey` 已处理 undefined-as-delete，本层简化为整段覆盖）
...
- ✅ 质量门：`pnpm type-check:tsgo` 0 errors；`pnpm exec eslint app/components/workflow/nodes/ensemble-aggregator/` 0 errors / 1 warning（pre-existing array-index key）；`pnpm test -- --run app/components/workflow/nodes/parallel-ensemble` 8 files / 75 cases 全绿（dynamic-config-form 共享体未动）
...
### ✅ P3.A.3 ensemble_aggregator 测试翻译 + 新增（1d，2026-04-30）
...
- ✅ 质量门：后端 `pytest ensemble_aggregator/ parallel_ensemble/` → 245 passed（ensemble_aggregator 净增 9 cases，77→86）；前端 `pnpm test ensemble-aggregator parallel-ensemble` → 10 files / 98 passed（parallel-ensemble 8 file / 75 case 不退化）；`pnpm type-check:tsgo` 0 errors；`pnpm exec eslint app/components/workflow/nodes/ensemble-aggregator/` 0 errors / 1 warning（pre-existing array-index key）
...
- `aggregators/response/*` **删除**（ADR-v3-9，response 路径转给 ensemble_aggregator）
...
- `_extract_variable_selector_to_variable_mapping` 暴露 prompt 模板的所有 var 引用（与 P1.3 ensemble_aggregator 对称）
