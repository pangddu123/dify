...
### P3.A.2 ensemble_aggregator 前端升级（1.5d）

依赖：P3.A.1

- `types.ts`：`AggregationInputRef` 加 `weight`（`number | VariableSelector`）/ `fallback_weight: number | null` / `extra: Record<string, unknown>`；**不**加 `top_k_override`；strategy 字面量从 2 个扩到 3+ 个
- `components/input-list.tsx` 行内加 weight 输入框（数字 OR 变量引用 picker）+ fallback_weight 数字输入框（默认空 = fail fast，行内 tooltip 说明 ADR-v3-15）
- `components/strategy-selector.tsx` 新增 `weighted_majority_vote` 选项 + ui_schema 反射策略私有 config（复用 P2.11 `dynamic-config-form`）
- `use-config.ts` 新增 `handleWeightChange` / `handleFallbackWeightChange`
- i18n（en-US + zh-Hans）：`nodes.ensembleAggregator.weight` / `fallbackWeight` / `weightedMajorityVote.*`（**不**加 `topKOverride`）
