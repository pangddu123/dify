...
import {
  RESPONSE_STRATEGY_META,
  RESPONSE_STRATEGY_NAMES,
} from '../types'
...
type Props = {
  ...
  onStrategyChange: (name: ResponseStrategyName) => void
  onStrategyConfigChange: (next: ResponseStrategyConfig) => void
}
...
const StrategySelector: FC<Props> = ({
  ...
  const meta = RESPONSE_STRATEGY_META[strategyName]
  ...
  return (
    <div className="space-y-3">
      <DropdownMenu open={open} onOpenChange={setOpen}>
        ...
        >
          {RESPONSE_STRATEGY_NAMES.map(name => (
            <DropdownMenuItem
              key={name}
