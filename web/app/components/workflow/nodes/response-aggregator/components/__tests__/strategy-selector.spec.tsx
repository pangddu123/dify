import type { ResponseStrategyConfig, ResponseStrategyName } from '../../types'
...
type FormProps = {
  ...
  uiSchema: UiSchema
  value: ResponseStrategyConfig
  readonly?: boolean
  onChange: (next: ResponseStrategyConfig) => void
}
...
type Handlers = {
  onStrategyChange: ReturnType<typeof vi.fn<(name: ResponseStrategyName) => void>>
  onStrategyConfigChange: ReturnType<typeof vi.fn<(next: ResponseStrategyConfig) => void>>
}
const renderSelector = (overrides: Partial<Handlers> & {
  strategyName?: ResponseStrategyName
  strategyConfig?: ResponseStrategyConfig
  readonly?: boolean
} = {}) => {
  const handlers: Handlers = {
    onStrategyChange: overrides.onStrategyChange ?? vi.fn<(name: ResponseStrategyName) => void>(),
    onStrategyConfigChange: overrides.onStrategyConfigChange ?? vi.fn<(next: ResponseStrategyConfig) => void>(),
  }
  render(
    <StrategySelector
...
describe('response-aggregator/strategy-selector', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    lastFormProps = null
  })
  ...
  describe('Dropdown surface', () => {
    ...
    it('shows the active strategy label on the trigger', () => {
      ...
      expect(trigger.textContent).toMatch(
        /nodes\.responseAggregator\.strategies\.concat\.label/,
      )
    })
  ...
  describe('Selection mutation', () => {
    ...
    it('does not fire onStrategyChange when re-selecting the active strategy', () => {
      const onStrategyChange = vi.fn<(name: ResponseStrategyName) => void>()
  ...
  describe('Schema reflection', () => {
    ...
    it('renders DynamicConfigForm with the concat ui_schema and i18n prefix', () => {
      ...
      expect(form.dataset.prefix).toBe(
        'nodes.responseAggregator.concat',
      )
    ...
    it('forwards form mutations to onStrategyConfigChange', () => {
      const onStrategyConfigChange = vi.fn<(next: ResponseStrategyConfig) => void>()
      renderSelector({
        strategyName: 'concat',
